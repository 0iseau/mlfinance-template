...
from mlfinance.models import gradient_boosting_model


def test_gb_target_column_missing() -> None:
    data = pd.DataFrame(
        {
    ...
    with pytest.raises(ValueError):
        gradient_boosting_model(data, target="returns")


def test_gb_output() -> None:
    data = pd.DataFrame(
        {
            "Date": [
                "2020-01-01",
    ...
    result, importance, summary = gradient_boosting_model(data, target="returns")
    ...
    assert summary["model"] == "Gradient Boosting"
...
def test_gb_invalid_data_and_target() -> None:
    with pytest.raises(ValueError):
        gradient_boosting_model([1, 2, 3], target="returns")  # type: ignore[arg-type]
    ...
    with pytest.raises(ValueError):
        gradient_boosting_model(data, target="invalid_target")
